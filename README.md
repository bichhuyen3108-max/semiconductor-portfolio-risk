...
본 프로젝트에서는 3개 반도체 종목으로 구성된 **동일가중 포트폴리오(Equal-weight Portfolio)**를 사용하였습니다.    
...
포트폴리오 수익률 분포에서 **VaR와 CVaR를 함께 시각화하여 Tail Risk 특성** 을 분석합니다.

<img src="results/figures/var_cvar_multi_levels.png" width="900">


분석 결과 포트폴리오 수익률 분포의 **왼쪽 꼬리 영역(Left Tail)** 에서 극단적인 손실이 관찰됩니다.

- **VaR** 는 특정 신뢰수준에서 발생할 수 있는 최대 손실 임계값을 의미합니다.
- **CVaR** 는 VaR를 초과하는 손실 구간의 평균 손실을 의미하며 **극단적 Tail Risk를 보다 정확하게 측정** 합니다.
...
이는 **최악의 5% 구간에서 평균 손실이 약 4% 수준까지 확대될 수 있음** 을 보여줍니다.

또한 **99% 수준에서는 Tail Risk가 더욱 확대** 되며, 이는 금융 수익률 분포가 **Fat Tail 특성** 을 가진다는 점과 일치합니다.
...
## 📉 VaR Backtesting Result (Kupiec Test)

VaR 모델의 예측 정확도를 평가하기 위해 **Kupiec POF(Proportion of Failures) Test** 를 수행하였습니다.

먼저 VaR 신뢰수준별 전체 Violation 빈도를 검증한 결과는 다음과 같습니다.   

| Confidence Level | Expected Rate | Violation Rate | Result |
| ---------------- | ------------- | -------------- | ------ |
| 95%              | 5%            | ~5.89%         | PASS   |
| 99%              | 1%            | ~1.56%         | FAIL   |

분석 결과 **VaR 95% 수준에서는 Kupiec Test를 통과(PASS)** 하였으며, 이는 모델이 **일반적인 시장 위험 수준(Typical Market Risk)** 을 비교적 잘 설명하고 있음을 의미합니다.

반면 **VaR 99% 수준에서는 Test를 통과하지 못했으며(FAIL)** , 이는 실제 시장에서 **극단적 손실(Extreme Loss)이 모델이 예측한 것보다 더 자주 발생함** 을 보여줍니다.

추가적으로 VaR 모델별 Violation 결과는 다음과 같습니다.

| Model         | Confidence Level | Violation Rate | Result |
| ------------- | ---------------- | -------------- | ------ |
| Student-t VaR | 95%              | ~6.47%         | FAIL   |
| GARCH VaR     | 95%              | ~5.94%         | FAIL   |
| Student-t VaR | 99%              | ~1.61%         | FAIL   |
| GARCH VaR     | 99%              | ~2.09%         | FAIL   |

모델별 분석 결과 **Student-t VaR와 GARCH VaR 모두 기대된 Violation 빈도보다 높은 값을 보였으며** , 이는 VaR 기반 모델이 **시장 스트레스 상황에서 발생하는 Tail Risk를 완전히 설명하기 어려울 수 있음** 을 시사합니다.

특히 이러한 결과는 금융 시장에서 자주 관찰되는 다음과 같은 특성과도 일치합니다.

- **Fat Tail Distribution**

- **Volatility Clustering**

즉, VaR 모델은 **일반적인 시장 변동성은 비교적 잘 설명하지만, 극단적인 시장 충격 구간에서는 실제 손실을 과소평가할 가능성** 이 있음을 보여줍니다.


## 📌 Interpretation

위의 Backtesting 결과는 **VaR 모델이 일반적인 시장 변동성은 비교적 잘 설명할 수 있지만, 극단적인 시장 충격 상황에서는 Tail Risk를 완전히 포착하지 못할 수 있음** 을 보여줍니다.

특히 **2020년 COVID 시장 충격 구간** 과 일부 **High Volatility Regime** 에서 VaR Violation이 집중적으로 발생한 점은 금융 시장 수익률이 **Fat Tail Distribution** 과 **Volatility Clustering** 특성을 가진다는 기존 금융 연구 결과와도 일치합니다.

따라서 본 분석 결과는 **포트폴리오 리스크가 단일 VaR 지표로만 설명되기 어렵고** , 시장 변동성 상태에 따라 리스크가 변화하는 **Dynamic Risk Structure** 를 고려한 리스크 관리가 필요함을 시사합니다.


## 🔍 VaR Violation Clustering Analysis

VaR Backtesting 결과, VaR violation은 무작위적으로 발생하기보다 특정 시기에 집중되는 경향을 보였습니다.

특히 **2020 COVID Market Shock** 및 **High Volatility Regime** 구간에서 violation이 군집적으로 나타났으며, 이는 금융 시장의 **Volatility Clustering** 특성과 일치합니다.
...
특히 **2020년 COVID 시장 충격 구간** 에서 **VaR Violation** 이 집중적으로 발생하며 금융 시장의 **Volatility Clustering** 특성이 확인되었습니다.

### 2️⃣ Historical VaR 특징

**Historical VaR** 는 **과거 수익률 분포(Historical Return Distribution)** 를 기반으로 하기 때문에 **급격한 시장 변동성 확대 구간** 에서는 **리스크 변화를 빠르게 반영하지 못하는 경향** 을 보였습니다.

### 3️⃣ Student-t VaR 특징

**Student-t VaR** 는 금융 수익률의 **Fat Tail 특성** 을 반영하여 **Historical VaR보다 극단적 손실 가능성(Extreme Loss Risk)** 을 보다 **보수적으로 추정** 하는 경향을 보였습니다.

### 4️⃣ GARCH VaR 특징

**GARCH VaR** 는 **조건부 변동성** 을 반영하기 때문에 **시장 변동성이 확대되는 구간** 에서 **VaR 값이 빠르게 확대되는 특징** 을 보였습니다.

### 5️⃣ Volatility Regime 

**GARCH Volatility Regime 분석 결과** 시장 변동성은 일정하지 않고 **Low / Moderate / High Regime** 사이를 **순환하는 특징** 을 보였습니다.

특히 **시장 충격 구간** 에서는 **High Volatility Regime** 이 집중적으로 나타나며
**포트폴리오 하방 리스크** 가 크게 확대되는 경향이 확인되었습니다.

### 6️⃣ Dynamic Risk Management Implication

분석 결과 **포트폴리오 리스크** 는 **고정된 값이 아니라 시장 변동성 상태(Market Volatility Regime)** 에 따라 **지속적으로 변화하는 특성** 을 보였습니다.

따라서 **정적인 리스크 관리 방식(Static Risk Management)** 보다 **시장 변동성 Regime** 에 따라 **포트폴리오 노출도(Portfolio Exposure)** 를 조절하는 **동적 리스크 관리(Dynamic Risk Management)** 가 필요합니다.
